...
public class BusinessSummaryEnrichmentLookup: EnrichmentLookup {

    public var results:[BusinessSummaryResult]?

    init(smartyKey: String){
        self.results = nil
        super.init(smartyKey: smartyKey, datasetName: "business", dataSubsetName: "")
    }

    init(lookup: EnrichmentLookup) {
        self.results = nil
        super.init(smartyKey: lookup.getSmartyKey(), datasetName: "business", dataSubsetName: "")
        ...
        self.setBusinessName(businessName: lookup.getBusinessName())
        self.setFeatures(features: lookup.getFeatures())
        self.setRequestEtag(etag: lookup.getRequestEtag())
    ...
    override public func deserializeAndSetResults(serializer: SmartySerializer, payload: Data, error: UnsafeMutablePointer<NSError?>) {
        self.results = serializer.Deserialize(payload: payload, error: &error.pointee) as? [BusinessSummaryResult]
    }
}
